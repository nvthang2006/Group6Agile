...
@section('title', 'Quản lý Danh mục - Tour Manager')
        ...
        <div class="widget-content widget-content-area br-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 mt-3">
                ...
                <div class="d-flex gap-2">
                    <a href="{{ route('categories.trash') }}" class="btn btn-outline-danger">
                ...
                <tbody>
                    @forelse ($categories as $category)
                        <tr>
                            ...
                            <td><span class="badge badge-light-info">{{ $category->slug }}</span></td>
                            <td>{{ Str::limit($category->description, 60) }}</td>
                            <td>
                                <div class="d-flex gap-2">
                                    <a href="{{ route('categories.show', $category->id) }}" class="btn btn-sm btn-info" title="Xem">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                                    </a>
                                    <a href="{{ route('categories.edit', $category->id) }}" class="btn btn-sm btn-warning" title="Sửa">
                                    ...
                                    <form action="{{ route('categories.destroy', $category->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa danh mục này không?')">
                                        @csrf
                                        @method('DELETE')
